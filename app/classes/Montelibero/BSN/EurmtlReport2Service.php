...
class EurmtlReport2Service
{
    private const CACHE_KEY_PREFIX = 'eurmtl_report2_snapshot:v3';
    ...
    public function __construct(
        ...
        private readonly EurmtlReport2Config $Config,
    ...
    private function buildSnapshot(array $config): array
    {
        ...
        $eurmtl_asset = $this->makeAsset(EurmtlReport2Config::EURMTL_CODE);
        $eurdebt_asset = $this->makeAsset(EurmtlReport2Config::EURDEBT_CODE);
        $asset_stats = [
            EurmtlReport2Config::EURMTL_CODE => $this->fetchAssetStats(EurmtlReport2Config::EURMTL_CODE),
            EurmtlReport2Config::EURDEBT_CODE => $this->fetchAssetStats(EurmtlReport2Config::EURDEBT_CODE),
        ];
        ...
        $treasury_ids = array_fill_keys(
            array_diff(
                ...
                array_merge(EurmtlReport2Config::MARKET_MAKER_ACCOUNTS, [EurmtlReport2Config::ISSUER, EurmtlReport2Config::BTPOOL])
            ),
            true
        );
        ...
        $tracked_account_ids = array_values(array_unique(array_merge(
            array_keys($eurdebt_holders),
            EurmtlReport2Config::MARKET_MAKER_ACCOUNTS,
            [EurmtlReport2Config::ISSUER, EurmtlReport2Config::BTPOOL],
            array_keys($pool_eurmtl_by_account)
        )));
        $eurmtl_holders = array_replace(
            $this->collectAssetHolders($eurmtl_asset),
        ...
        $market_holders = $this->buildMarketHolders(
            ...
            $treasury_ids,
            $asset_stats[EurmtlReport2Config::EURMTL_CODE]['direct_balances']
        );
        $market_holders = $this->appendOffAccountMarketHolders(
            $market_holders,
            $claimable_eurmtl['total'],
            $asset_stats[EurmtlReport2Config::EURMTL_CODE]['contracts_amount']
        );
        ...
        $market_maker = $this->buildGroupBlock('market_maker', EurmtlReport2Config::MARKET_MAKER, EurmtlReport2Config::MARKET_MAKER_ACCOUNTS, $accounts, $pools, EurmtlReport2Config::MARKET_MAKER);
        $btpool = $this->buildGroupBlock('btpool', EurmtlReport2Config::BTPOOL, [EurmtlReport2Config::BTPOOL], $accounts, $pools);
        $fund = $this->buildGroupBlock('issuer', EurmtlReport2Config::ISSUER, [EurmtlReport2Config::ISSUER], $accounts, $pools);
        $market_maker['portfolio'] = $this->collectMarketMakerPortfolio(array_column($market_maker['direct']['accounts'], 'id'), $pools);
        $market_maker['valuation'] = $this->buildMarketMakerValuation($market_maker['portfolio']);
        $btpool['portfolio'] = $this->collectMarketMakerPortfolio(array_column($btpool['direct']['accounts'], 'id'), $pools);
        $btpool['valuation'] = $this->buildMarketMakerValuation($btpool['portfolio']);
        $fund['portfolio'] = $this->collectMarketMakerPortfolio(array_column($fund['direct']['accounts'], 'id'), $pools);
        $fund['valuation'] = $this->buildMarketMakerValuation($fund['portfolio']);

        $totals = $this->buildTotals($accounts, $pools, $asset_stats, $market_maker, $btpool, $fund, $market_holders, $claimable_eurmtl);
        ...
        return [
            ...
            'fund' => $fund,
            'claimable_balances' => [
                EurmtlReport2Config::EURMTL_CODE => $claimable_eurmtl,
                EurmtlReport2Config::EURDEBT_CODE => $claimable_eurdebt,
            ],
            'unknown' => [
                'claimable_eurmtl' => $claimable_eurmtl['total'],
                'contracts_eurmtl' => $asset_stats[EurmtlReport2Config::EURMTL_CODE]['contracts_amount'],
                'pool_unclassified_eurmtl' => $totals['pool_unclassified_eurmtl'],
            ],
            'metrics' => $this->buildMetrics($totals, $market_maker, $btpool, $fund),
            'totals' => $totals,
        ];
    }
    ...
    private function buildMarketHolders(array $eurmtl_holders, array $pool_eurmtl_by_account, array $treasury_ids, string $total_direct_eurmtl): array
    {
        ...
        foreach ($account_ids as $account_id) {
            $classification = $this->classifyAccount($account_id, $treasury_ids);
            if (in_array($classification, ['issuer', 'legacy_treasury', 'market_maker', 'btpool'], true)) {
        ...
        foreach ($rows as $row) {
            ...
            $totals['eurmtl_other'] = bcadd($totals['eurmtl_other'], $row['eurmtl_other'], self::SCALE);
    ...
    private function appendOffAccountMarketHolders(array $market_holders, string $claimable_amount, string $contracts_amount): array
    {
        foreach ([
            ['label' => 'Claimable balances', 'amount' => $claimable_amount],
            ['label' => 'В контрактах', 'amount' => $contracts_amount],
        ] as $item) {
            if (bccomp($item['amount'], '0', self::SCALE) <= 0) {
                continue;
            }

            $market_holders['rows'][] = [
                'account' => null,
                'classification' => 'off_account',
                'label' => $item['label'],
                'eurmtl_direct' => '0.0000000',
                'eurmtl_pool' => '0.0000000',
                'eurmtl_other' => $item['amount'],
                'eurmtl_total' => $item['amount'],
                'is_group' => true,
            ];
            $market_holders['totals']['eurmtl_other'] = bcadd($market_holders['totals']['eurmtl_other'] ?? '0.0000000', $item['amount'], self::SCALE);
            $market_holders['totals']['eurmtl_total'] = bcadd($market_holders['totals']['eurmtl_total'] ?? '0.0000000', $item['amount'], self::SCALE);
        }

        return $market_holders;
    }
    ...
    private function collectPools(array $treasury_ids): array
    {
        ...
        $Page = $this->Stellar
            ->liquidityPools()
            ->forReserves($this->canonicalAsset(EurmtlReport2Config::EURMTL_CODE))
            ->limit(200)
            ->execute();
    ...
    private function buildPool(LiquidityPoolResponse $Pool, array $treasury_ids): array
    {
        ...
        foreach ($Pool->getReserves()->toArray() as $Reserve) {
            ...
            if ($asset['key'] === $this->assetKey(EurmtlReport2Config::EURMTL_CODE)) {
                $eurmtl_reserve = $amount;
            } elseif ($asset['key'] === $this->assetKey(EurmtlReport2Config::EURDEBT_CODE)) {
    ...
    {
        $account_ids = EurmtlReport2Config::MARKET_MAKER_ACCOUNTS;
        ...
        $totals = [
            'account' => $this->accountData(EurmtlReport2Config::MARKET_MAKER),
        ...
        return [
            ...
            'offers' => $this->collectOffers(EurmtlReport2Config::MARKET_MAKER),
        ];
    }

    private function buildGroupBlock(string $classification, string $primary_account_id, array $account_ids, array $accounts, array $pools, ?string $offer_account_id = null): array
    {
        $account_id_map = array_fill_keys($account_ids, true);
        $account_rows = [];
        ...
        foreach ($accounts as $account) {
            ...
            if ($account_id && isset($account_id_map[$account_id])) {
                $account_rows[$account_id] = $account;
                $account_rows[$account_id]['classification'] = $classification;
            }
        }
        ...
        $totals = [
            'account' => $this->accountData($primary_account_id),
            'accounts' => array_map(static fn(array $account): array => $account['account'], array_values($account_rows)),
            'classification' => $classification,
            'eurmtl_direct' => '0.0000000',
            'eurmtl_pool' => '0.0000000',
        ...
        return [
            ...
            'direct' => $totals,
            'pool_positions' => $pool_positions,
            'offers' => $offer_account_id ? $this->collectOffers($offer_account_id) : [],
        ];
    }
    ...
    private function fetchStrictSendEurmtlRate(array $asset): ?string
    {
        ...
        $response = $this->fetchJson('https://horizon.stellar.org/paths/strict-send', [
            ...
            'source_amount' => '1',
            'destination_assets' => EurmtlReport2Config::EURMTL_CODE . ':' . EurmtlReport2Config::ISSUER,
        ]);
    ...
    private function fetchStrictReceiveEurmtlRate(array $asset): ?string
    {
        ...
        $response = $this->fetchJson('https://horizon.stellar.org/paths/strict-receive', [
            'destination_asset_type' => $this->horizonAssetType(EurmtlReport2Config::EURMTL_CODE),
            'destination_asset_code' => EurmtlReport2Config::EURMTL_CODE,
            'destination_asset_issuer' => EurmtlReport2Config::ISSUER,
    ...
    private function buildTotals(array $accounts, array $pools, array $asset_stats, array $market_maker, array $btpool, array $fund, array $market_holders, array $claimable_eurmtl): array
    {
        $legacy_market = '0.0000000';
        $direct_exposure = '0.0000000';
        $treasury_eurdebt = '0.0000000';
        $treasury_eurmtl_direct = '0.0000000';
        $treasury_eurmtl_pool = '0.0000000';
        foreach ($accounts as $account) {
            if (($account['classification'] ?? null) === 'legacy_treasury') {
                ...
                $treasury_eurdebt = bcadd($treasury_eurdebt, $account['eurdebt_direct'], self::SCALE);
                $treasury_eurmtl_direct = bcadd($treasury_eurmtl_direct, $account['eurmtl_direct'], self::SCALE);
                $treasury_eurmtl_pool = bcadd($treasury_eurmtl_pool, $account['eurmtl_pool'], self::SCALE);
            }
        }
        $market_maker_market = $market_maker['direct']['market_amount'] ?? '0.0000000';
        $btpool_market = $btpool['direct']['market_amount'] ?? '0.0000000';
        $debt_gap_market = bcadd(bcadd($legacy_market, $market_maker_market, self::SCALE), $btpool_market, self::SCALE);
        ...
        $contracts_eurmtl = $asset_stats[EurmtlReport2Config::EURMTL_CODE]['contracts_amount'] ?? '0.0000000';
        ...
        $preliminary = $market_holders_amount;
        $coverage_eur = bcadd(
            bcadd($legacy_market, $market_maker['valuation']['total_eur'] ?? '0.0000000', self::SCALE),
            bcadd($btpool['valuation']['total_eur'] ?? '0.0000000', $fund['valuation']['total_eur'] ?? '0.0000000', self::SCALE),
            self::SCALE
        );
        $fund_balance = bcsub($coverage_eur, $preliminary, self::SCALE);

        return [
            'legacy_treasury_market_amount' => $legacy_market,
            'treasury_eurdebt' => $treasury_eurdebt,
            'treasury_eurmtl_direct' => $treasury_eurmtl_direct,
            'treasury_eurmtl_pool' => $treasury_eurmtl_pool,
            'market_maker_market_amount' => $market_maker_market,
            'btpool_market_amount' => $btpool_market,
            ...
            'coverage_eur' => $coverage_eur,
            'fund_balance_eur' => $fund_balance,
            'fund_buyback_required_eur' => bccomp($fund_balance, '0', self::SCALE) < 0 ? bcsub('0', $fund_balance, self::SCALE) : '0.0000000',
            'fund_can_emit_eur' => bccomp($fund_balance, '0', self::SCALE) > 0 ? $fund_balance : '0.0000000',
    ...
    private function buildMetrics(array $totals, array $market_maker, array $btpool, array $fund): array
    {
        return [
            'market_eurmtl' => $totals['preliminary_market_estimate'] ?? '0.0000000',
            'treasury_cash_eur' => $totals['legacy_treasury_market_amount'] ?? '0.0000000',
            'market_maker_issued_eurmtl' => $totals['market_maker_market_amount'] ?? '0.0000000',
            'market_maker_coverage_eur' => $market_maker['valuation']['total_eur'] ?? '0.0000000',
            'btpool_issued_eurmtl' => $totals['btpool_market_amount'] ?? '0.0000000',
            'btpool_coverage_eur' => $btpool['valuation']['total_eur'] ?? '0.0000000',
            'fund_coverage_eur' => $fund['valuation']['total_eur'] ?? '0.0000000',
            'coverage_eur' => $totals['coverage_eur'] ?? '0.0000000',
            'fund_balance_eur' => $totals['fund_balance_eur'] ?? '0.0000000',
            'claimable_eurmtl' => $totals['claimable_eurmtl'] ?? '0.0000000',
            'contracts_eurmtl' => $totals['contracts_eurmtl'] ?? '0.0000000',
            'pool_market_eurmtl' => $totals['non_issuer_pool_eurmtl'] ?? '0.0000000',
        ];
    }
    ...
    {
        $holders = [];
        $asset_id = rawurlencode($code . '-' . EurmtlReport2Config::ISSUER);
    ...
    private function fetchAssetStats(string $code): array
    {
        $Page = $this->Stellar
            ->assets()
            ->forAssetCode($code)
            ->forAssetIssuer(EurmtlReport2Config::ISSUER)
            ->limit(1)
            ->execute();
        ...
        return [
            'code' => $code,
            'issuer' => EurmtlReport2Config::ISSUER,
    ...
    {
        if ($account_id === EurmtlReport2Config::ISSUER) {
            return 'issuer';
        }
        if (in_array($account_id, EurmtlReport2Config::MARKET_MAKER_ACCOUNTS, true)) {
            return 'market_maker';
        }
        if ($account_id === EurmtlReport2Config::BTPOOL) {
            return 'btpool';
        }
    ...
    {
        $eurmtl_key = $this->assetKey(EurmtlReport2Config::EURMTL_CODE);
    ...
    {
        return Asset::createNonNativeAsset($code, EurmtlReport2Config::ISSUER);
    }

    private function canonicalAsset(string $code): string
    {
        return $code . ':' . EurmtlReport2Config::ISSUER;
    }

    private function assetKey(string $code, ?string $issuer = EurmtlReport2Config::ISSUER): string
    {
